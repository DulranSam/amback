...
Router.route("/")
  .post(upload.single("media"), async (req, res) => {
    ...
    if (!title || !description || !link || !category || !commission || !mediaBuffer) {
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      if (selectedType === "all" || selectedType) {
        ...
        data = await mainModel.find({ category: selectedType }).sort({ createdAt: -1 });
      }
...
Router.route("/:id").put(async (req, res) => {
  ...
  const { title, description, mediaUrl, link, category, commission, gmail, password } = req.body;

  if (!id || !gmail || !password)
    return res.status(400).json({ Alert: "ID and user authentication required" });
  ...
  try {
    ...
    if (data.userId !== userId) { // Assuming data.userId holds the user ID who added the item
      return res.status(401).json({ Alert: "Unauthorized. You are not allowed to modify this item." });
    }

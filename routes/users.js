...
Router.route("/register").post(async (req, res) => {
  ...
  try {
    ...
    // Uncomment the email sending code if needed
    // const mailOptions = {
    //   from: EMAIL_USER,
    //   to: newUser.gmail,
    ...
    if (newUser) {
      ...
      return res
        .status(403)
        .json({ Error: `Error while creating user with ${gmail}` });
    }
  } catch (err) {
    console.error(err);
...
Router.route("/forgot/:id").put(async (req, res) => {
  ...
  if (!password) return res.status(400).json({ Alert: "Password required" });
